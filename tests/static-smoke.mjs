...
try{
  ...
  if(!html.includes('六爻玄機引擎')||!html.includes('事後回測')||!html.includes('伏神／飛神分析')||!html.includes('id="hiddenSpirits"')||!html.includes('id="useGodRuleTable"')||!html.includes('v1.4'))throw Error('首頁 v1.4 內容不正確');
  for(const path of ['/v1.2.css','/v1.3.css','/src/engine/hiddenSpirits.js','/rules/source-catalog.json','/rules/rule-pack.json']){const asset=await fetch(`http://127.0.0.1:${port}${path}`);if(!asset.ok)throw Error(`${path} 無法載入`)}
